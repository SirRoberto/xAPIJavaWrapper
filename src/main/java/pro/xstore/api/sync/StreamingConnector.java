...
public class StreamingConnector extends Connector {
                ...
                while (streamConnected) {
                    try {
                        ...
                        if ((message = readMessageFromStream()) != null) {
                            ...
                            if (jsonMsg.has("command") && jsonMsg.has("data")) {
    ...
    private static BaseResponseRecord createResponseRecord(String command) {
        ...
        if (command != null) {
            switch (command) {
                case "tickPrices" -> result = new STickRecord();
                case "trade" -> result = new STradeRecord();
                case "balance" -> result = new SBalanceRecord();
                case "tradeStatus" -> result = new STradeStatusRecord();
                case "profit" -> result = new SProfitRecord();
                case "news" -> result = new SNewsRecord();
                case "keepAlive" -> result = new SKeepAliveRecord();
                case "candle" -> result = new SCandleRecord();
            }
        }
        return result;
    }
    ...
    private void invokeListener(String command, BaseResponseRecord brr) {
        if (command != null) {
            switch (command) {
                case "tickPrices" -> sl.receiveTickRecord((STickRecord) brr);
                case "trade" -> sl.receiveTradeRecord((STradeRecord) brr);
                case "balance" -> sl.receiveBalanceRecord((SBalanceRecord) brr);
                case "tradeStatus" -> sl.receiveTradeStatusRecord((STradeStatusRecord) brr);
                case "profit" -> sl.receiveProfitRecord((SProfitRecord) brr);
                case "news" -> sl.receiveNewsRecord((SNewsRecord) brr);
                case "keepAlive" -> sl.receiveKeepAliveRecord((SKeepAliveRecord) brr);
                case "candle" -> sl.receiveCandleRecord((SCandleRecord) brr);
            }
        }
    }
